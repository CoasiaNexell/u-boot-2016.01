...
#define CONFIG_SYS_PROMPT			"s5p4418_drone_ref# "
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS                                          \
	...
	"else number=$loop; "                                                  \
	"fi; "                                                                 \
	"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-drone_ref.dtb" \
	" && setexpr success 1; " \
	"setexpr loop $loop - 1; "                                             \
	"done; "                                                               \
	...
	"boot_cmd_ramfsboot=ext4load mmc 0:1 0x71008000 zImage; " \
		...
		"ext4load mmc 0:1 0x7A000000 s5p4418-drone_ref.dtb; " \
		"run dtb_reserve; bootz 0x71008000 0x79000000 0x7A000000\0" \
	"ramfsboot=setenv "         \
	"bootargs "                 \
	"console=ttyAMA0,115200n8 root=/dev/ram0 loglevel=4 printk.time=1" \
	" consoleblank=0 nx_drm.fb_buffers=3; run boot_cmd_ramfsboot \0" \
